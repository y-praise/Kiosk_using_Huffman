...
// priorty_queue에서 우선순위 계산 위해 operator()도 정의해야 함
class HNode {
public:
	...
	// priority_queue에서 빈도수 비교를 위한 operator() 오버로딩
	bool operator()(HNode* a, HNode* b) {
		return a->frequency > b->frequency;		// 빈도수가 낮은 것이 우선순위 높음
	}
	// getter/setter
...
private:
	...
	vector<LinkedList> orderLists;		// 주문 리스트의 헤드 포인터
